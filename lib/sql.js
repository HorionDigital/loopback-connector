...
SQLConnector.prototype.idColumnEscaped = function(model) {
  return model + "." + this.escapeName(this.idColumn(model));
};
...
SQLConnector.prototype.tableEscaped = function(model) {
  return this.escapeName(this.table(model)) + " AS " + model;
};
...
SQLConnector.prototype._buildFieldsForKeys = function(model, data, keys, excludeIds) {
  ...
  for (var i = 0, n = keys.length; i < n; i++) {
    ...
    if(typeof k == "string" && k.startsWith(model + ".")) {
      k = k.replace(model + ".", "");
    }
...
SQLConnector.prototype.buildSelect = function(model, filter, options) {
  ...
  if (filter) {
    ...
    var relationsWhere = new ParameterizedSQL();

    if(filter.leftJoin) {
      var leftJoinWhere = this.buildJoin(selectStmt, "LEFT JOIN", filter.leftJoin, model, filter, options);
      relationsWhere.merge(leftJoinWhere);
    }
    if(filter.rightOuterJoin) {
      var rightOuterJoinWhere = this.buildJoin(selectStmt, "RIGHT OUTER JOIN", filter.rightOuterJoin, model, filter, options);
      if(relationsWhere && relationsWhere.sql && rightOuterJoinWhere && rightOuterJoinWhere.sql) {
        relationsWhere.sql += " AND ";
      }
      relationsWhere.merge(rightOuterJoinWhere);
    }

    let addWhereForRelations = true;
    let simpleWhereExists = false;

    if (filter.where) {
      var whereStmt = this.buildWhere(model, filter.where);
      if(whereStmt && whereStmt.sql) {
        addWhereForRelations = !whereStmt.sql.startsWith("WHERE");
        simpleWhereExists = true;
      }
      selectStmt.merge(whereStmt);
    }

    if(filter.leftJoin || filter.rightOuterJoin) { // OR other joins
      if(relationsWhere && relationsWhere.sql) {
        if(addWhereForRelations) {
          selectStmt.sql += "WHERE ";
        }
        if(simpleWhereExists) {
          selectStmt.sql += " AND ";
        }
      }
      selectStmt.merge(relationsWhere);
      var thisIdName = this.idName(model);
      selectStmt.sql += " GROUP BY " + this.columnEscaped(model, thisIdName);
    }
...
SQLConnector.prototype.buildJoin = function(selectStmt, joinType, join, model, filter, options) {

    var relationsWhere;
  
    if(!join) {
      return relationsWhere;
    }
  
    var relations;
  
    try{
      relations = this.getModelDefinition(model).settings.relations;
    } catch(e) {
    }
  
    for(var key in join) {
      if(join[key] && relations && relations[key]) {
  
        var realtion = relations[key];
  
        var relIdName = this.idName(realtion.model);
        var thisIdName = this.idName(model);
  
        selectStmt.sql += " " + joinType + " " + this.tableEscaped(realtion.model)
        + " ON "
          + (realtion.type == "belongsTo" ? this.columnEscaped(realtion.model, relIdName) : this.columnEscaped(realtion.model, realtion.foreignKey))
          + "="
          + (realtion.type == "belongsTo" ? this.columnEscaped(model, realtion.foreignKey) : this.columnEscaped(model, thisIdName))
        ;
  
        if(!relationsWhere) {
          relationsWhere = new ParameterizedSQL();
        }
  
        var relWhereStmt = this.buildWhere(realtion.model, join[key]);
        relationsWhere.merge(relWhereStmt);
  
        if(relationsWhere && relationsWhere.sql && relationsWhere.sql.startsWith("WHERE")) {
          relationsWhere.sql = relationsWhere.sql.substring(6);
        }
      }
    }
